...
*/
class Prettylittlethingcom_Commercebug_Model_Collectorsysteminfo extends Prettylittlethingcom_Commercebug_Model_Observingcollector
{
    protected $_items;
    public function collectInformation($observer)
    {
        ...
        {
            Prettylittlethingcom_Commercebug_Model_Crossareaajax::validateControllers();
        }
        
        $system_info->ajax_paths = new stdClass;
        $system_info->form_key = $this->getShim()->getSingleton('core/session')->getFormKey();
        foreach(Prettylittlethingcom_Commercebug_Model_Crossareaajax::getAjaxActionNames() as $action)
        {
    ...
    public function createAjaxUrl($action)
    {
        ...
        #Prettylittlethingcom_Commercebug_Model_Shim::Log('Mage2 Cleanup in ' . __CLASS__ . ' ' . __LINE__);
    ...
    */		
    public function getUrl()
    {
        ...
        #Prettylittlethingcom_Commercebug_Model_Shim::Log('Cleanup in ' . __CLASS__ . ' ' . __LINE__);
        if($alias=='adminhtml/url' && $shim->isMage2())
        {
            $alias = 'backend/url';
        }
